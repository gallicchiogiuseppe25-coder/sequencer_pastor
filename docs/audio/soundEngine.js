...
(function () {
  ...
  let silentKeepAliveEl = null;  // <audio> silente in loop: sblocca le casse iOS col mute attivo

  /**
   * Genera un WAV silenzioso di ~0.5s interamente in memoria (nessun file
   * esterno) e lo riproduce in loop tramite un elemento <audio>. Deve essere
   * chiamata dentro un gesture utente. Vedi il commento in init() per il
   * perché: aggancia la sessione audio iOS alla categoria "playback".
   */
  function startSilentKeepAliveAudio() {
    try {
      ...
      // I campioni restano a zero: silenzio assoluto.

      const blob = new Blob([buffer], { type: 'audio/wav' });
      silentKeepAliveEl = document.createElement('audio');
      silentKeepAliveEl.src = URL.createObjectURL(blob);
      silentKeepAliveEl.loop = true;
      silentKeepAliveEl.setAttribute('playsinline', 'true');
      // Volume minimo ma NON zero né muted: un media element mutato non
      // aggancia la categoria playback su iOS.
      silentKeepAliveEl.volume = 0.01;
      const p = silentKeepAliveEl.play();
      if (p && p.catch) p.catch(() => { /* se fallisce, il resto funziona comunque come prima */ });
    } catch (e) { /* non critico: senza sblocco si torna al comportamento precedente */ }
  }
  ...
  function init() {
    ...
    // --- Sblocco casse su iOS -------------------------------------------
    // Su iPhone, Web Audio nasce in categoria "ambient": rispetta
    // l'interruttore laterale del silenzioso, quindi con il telefono in
    // mute l'audio esce SOLO da cuffie/casse esterne, mai dagli speaker.
    // Le due contromisure (complementari, entrambe innocue altrove):
    // 1. AudioSession API (iOS 17+): chiede esplicitamente la categoria
    //    "playback", che ignora l'interruttore mute.
    // 2. Un elemento <audio> HTML silenzioso in loop, avviato dentro lo
    //    stesso gesture utente: gli elementi media appartengono di natura
    //    alla categoria "playback" e agganciano a quella categoria l'intera
    //    sessione audio della pagina, Web Audio incluso. È la tecnica
    //    standard usata dai giochi web per suonare con il mute attivo.
    try {
      if (navigator.audioSession && 'type' in navigator.audioSession) {
        navigator.audioSession.type = 'playback';
      }
    } catch (e) { /* API sperimentale: ignorare in caso di errore */ }

    startSilentKeepAliveAudio();

    // Su iOS l'AudioContext viene sospeso/interrotto da telefonate, cambio
    // app, blocco schermo: al ritorno va ripreso esplicitamente, altrimenti
    // il sistema resta muto anche se tutto sembra attivo.
    ...
    // NOTA: il bus di saturazione (kickSaturationNode) esiste ancora ma è
    // BYPASSATO: era pensato per scaldare il kick, ma sul beep da telegrafo
    // aggiungerebbe armoniche sporcando il tono puro del La. Le voci vanno
    // dirette al master.
    kickVoices = [];
    for (let i = 0; i < NUM_VOICES; i++) {
